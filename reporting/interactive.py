...
def _add_trade_markers(fig: go.Figure, positions: pd.DataFrame, strat_equity: pd.Series) -> None:
    ...
    for label, mask, symbol, color in (
        ("buy", delta.gt(0).any(axis=1), "triangle-up", "green"),
        ("sell", delta.lt(0).any(axis=1), "triangle-down", "firebrick"),
    ):
        ...
        fig.add_trace(go.Scatter(
            ...
            customdata=[_trade_text(delta.loc[d]) for d in days],
...
def _trade_text(delta_row: pd.Series) -> str:
    return "<br>".join(
        f"{t} {'+' if d > 0 else ''}{d:.0f} sh" for t, d in delta_row.items() if abs(d) > 1e-9
    )
